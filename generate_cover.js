...
function color_the_greyed_S(fumens) { // array of fumens
    let results = [];
    ...
    (async () => {
        for await (const line of rl) {
            ...
            if (lineCount <= 84) {
                ...
                fumens = color_the_greyed_S(fumens);
                fumens = [...new Set(fumens)];
                glued_fumens = glue(fumens);

                all_fumens.push(...glued_fumens);

                let name = `S-s-${String(lineCount).padStart(2, '0')}`;
        ...
        let command = `java -jar sfinder.jar cover -K +t -d 180 -p 'S,*!' -t '${all_fumens.join(' ')}' --mode tss`
        console.log(command);
